...
class Database {
  ...
  public function hasUser($user) {
    $connection = mysqli_connect('localhost','Admin','password','Eitf05') or die ('Could not connect');
    $stmt = $connection->prepare("SELECT * FROM Users WHERE username COLLATE latin1_general_cs =?");
  ...
  public function isAuthenticated($user, $pass) {
    $connection = mysqli_connect('localhost','Admin','password','Eitf05') or die ('Could not connect');
    $stmt = $connection->prepare("SELECT salt FROM Users WHERE username COLLATE latin1_general_cs =?");
    $stmt->bind_param('s', $user);
    $stmt->execute();
    $res = $stmt->get_result();
    if(mysqli_num_rows($res) > 0) {
      $hashedPass = hash('sha256',$res->fetch_row()[0].$pass);
      $stmt = $connection->prepare("SELECT * FROM Users WHERE username COLLATE latin1_general_cs =? AND password COLLATE latin1_general_cs =?");
